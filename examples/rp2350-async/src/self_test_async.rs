//! Self-test example for ICM-20948 on Raspberry Pi Pico 2 (Async version)
//!
//! This example demonstrates:
//! - Running async hardware self-test for accelerometer
//! - Running async hardware self-test for gyroscope
//! - Running async hardware self-test for magnetometer
...
async fn main(_spawner: Spawner) {
    info!("========================================");
    info!("ICM-20948 Self-Test Example (Async)");
    info!("========================================");
    info!("");
    ...
    let mut imu = match Icm20948Driver::new(interface).await {
        Ok(imu) => {
    ...
    match imu.accelerometer_self_test(&mut delay).await {
        Ok(passed) => {
            if passed {
    ...
    match imu.gyroscope_self_test(&mut delay).await {
        Ok(passed) => {
            if passed {
    ...
    match imu.init_magnetometer(mag_config, &mut delay).await {
        Ok(_) => {
            info!("   ✓ Magnetometer initialized");
            Timer::after_millis(100).await;

            info!("   Applying internal test field to magnetometer");
            match imu.magnetometer_self_test(&mut delay).await {
                Ok(passed) => {
                    if passed {
