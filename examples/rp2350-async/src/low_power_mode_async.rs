//! Low-Power Mode Example for ICM-20948 on Raspberry Pi Pico 2 (Async version)
//!
...
async fn main(_spawner: Spawner) {
    ...
    for i in 0..20 {
        ...
        match (imu.read_accelerometer().await, imu.read_gyroscope().await) {
            (Ok(accel), Ok(gyro)) => {
                info!(
    ...
    if let Err(e) = imu.enter_low_power_mode(&lp_config, &mut delay).await {
    ...
    if let Err(e) = imu.exit_low_power_mode().await {
