//! Nine-axis sensor reading example for ICM-20948 on Raspberry Pi Pico 2 (Async version)
//!
...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        // Read accelerometer
        let accel_data = match imu.read_accelerometer().await {
            Ok(data) => {
        ...
        // Read gyroscope
        let gyro_data = match imu.read_gyroscope().await {
            Ok(data) => {
        ...
        // Read magnetometer
        let _mag_data = match imu.read_magnetometer().await {
            Ok(data) => {
                let magnitude = data.magnitude();
