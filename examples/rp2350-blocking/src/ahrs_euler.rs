...
async fn main(_spawner: Spawner) {
    info!("ICM-20948 AHRS Euler Angles for Tracked Robot (Blocking)");
    ...
    embassy_time::block_for(embassy_time::Duration::from_millis(3000));

    info!("Starting AHRS processing at {} Hz", SAMPLE_RATE_HZ);
    info!("");

    let mut sample_count: u32 = 0;
    let sample_period_ms = (1000.0 / SAMPLE_RATE_HZ) as u64;
    ...
    loop {
        ...
        if calibration_samples < CALIBRATION_COUNT {
            ...
            embassy_time::block_for(embassy_time::Duration::from_millis(sample_period_ms));
            continue;
        }
        ...
        if let Ok(quat) = result {
            ...
            // Output every 10th sample (10 Hz display rate) - clean and readable
